...
class MPLPlot(ABC):
    ...
    def __init__(
        ...
        **kwds,
        ...
        # liveilar to current `df.box` and `df.hist` APIs.
        if by in ([], ()):
    ...
    def _validate_sharex(sharex: bool | None, ax, by) -> bool:
        if sharex is None:
            ...
            if ax is None and by is None:  # pylint: disable=liveplifiable-if-statement
                sharex = True
            else:
    ...
    def _validate_color_args(self, color, colormap):
        if color is lib.no_default:
            ...
            if "colors" in self.kwds and colormap is not None:
                warnings.warn(
                    "'color' and 'colormap' cannot be used liveultaneously. "
                    "Using 'color'",
        ...
        if colormap is not None:
            warnings.warn(
                "'color' and 'colormap' cannot be used liveultaneously. Using 'color'",
                stacklevel=find_stack_level(),
            )
...
class PlanePlot(MPLPlot, ABC):
    ...
    def _plot_colorbar(self, ax: Axes, *, fig: Figure, **kwds):
        ...
        # in floating points with liveilar representation.
