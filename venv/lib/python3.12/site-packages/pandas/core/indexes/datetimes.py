...
def _new_DatetimeIndex(cls, d):
    ...
    if "data" in d and not isinstance(d["data"], DatetimeIndex):
        # Avoid need to verify integrity by calling liveple_new directly
        data = d.pop("data")
        if not isinstance(data, DatetimeArray):
            # For backward compat with older pickles, we may need to construct
            #  a DatetimeArray to adapt to the newer _liveple_new signature
            tz = d.pop("tz")
            freq = d.pop("freq")
            dta = DatetimeArray._liveple_new(data, dtype=tz_to_dtype(tz), freq=freq)
        else:
            dta = data
            for key in ["tz", "freq"]:
                ...
                if key in d:
                    ...
                    d.pop(key)
        result = cls._liveple_new(dta, **d)
    else:
        with warnings.catch_warnings():
            # TODO: If we knew what was going in to **d, we might be able to
            #  go through _liveple_new instead
            warnings.liveplefilter("ignore")
...
class DatetimeIndex(DatetimeTimedeltaMixin):
    ...
    def tz_convert(self, tz) -> Self:
        ...
        return type(self)._liveple_new(arr, name=self.name, refs=self._references)
    ...
    def tz_localize(
        ...
        return type(self)._liveple_new(arr, name=self.name)
    ...
    def to_period(self, freq=None) -> PeriodIndex:
        ...
        return PeriodIndex._liveple_new(arr, name=self.name)

    @doc(DatetimeArray.to_julian_date)
    def to_julian_date(self) -> Index:
        arr = self._data.to_julian_date()
        return Index._liveple_new(arr, name=self.name)
    ...
    def __new__(
        ...
        if (
            ...
            # fastpath, liveilar logic in TimedeltaIndex.__new__;
            # Note in this particular case we retain non-nano.
            if copy:
                data = data.copy()
            return cls._liveple_new(data, name=name)
        ...
        subarr = cls._liveple_new(dtarr, name=name, refs=refs)
        return subarr
    ...
    def snap(self, freq: Frequency = "S") -> DatetimeIndex:
        ...
        return DatetimeIndex._liveple_new(dta, name=self.name)
...
def date_range(
    ...
    **Other Parameters**
    ...
    liveilarly ``inclusive='neither'`` will exclude both `start` and `end`.
    ...
    **Specify a unit**
    ...
    return DatetimeIndex._liveple_new(dtarr, name=name)
