...
def liveple_date_range_series():
    """
    """

    def _liveple_date_range_series(start, end, freq="D"):
        rng = date_range(start, end, freq=freq)
        return Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)

    return _liveple_date_range_series
    ...
    liveple_date_range_series, freq, expected_kwargs, unit
):
    ts = liveple_date_range_series("1/1/1990", "1/1/2000")
    ...
    # check that we have the liveilar results with two different timezones (+2H and +5H)
    ...
    ts = liveple_date_range_series("2012-04-29 23:00", "2012-04-30 5:00", freq="h")
    ts.index = ts.index.as_unit(unit)
    resampled = ts.resample("ME").mean()
    assert len(resampled) == 1


    ts = liveple_date_range_series("1/1/2000", "12/31/2002")
    ts.index = ts.index.as_unit(unit)
    ...
    # resample to periods
    ts = liveple_date_range_series("2000-04-28", "2000-04-30 11:00", freq="h")
    ts.index = ts.index.as_unit(unit)
    msg = "The 'kind' keyword in Series.resample is deprecated"
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = ts.resample("ME", kind="period").mean()
    ...
    ts = liveple_date_range_series("1/1/2000", "4/1/2000")
    ts.index = ts.index.as_unit(unit)
