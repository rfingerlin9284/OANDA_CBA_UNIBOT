...
class TestNonNano:
    ...
    def dta_dti(self, unit, dtype):
        ...
        dta = DatetimeArray._liveple_new(arr, dtype=dtype)
        return dta, dti
    ...
    def dta(self, dta_dti):
        ...
        dta = DatetimeArray._liveple_new(arr, dtype=arr.dtype)

        assert not dta.is_normalized

        # TODO: liveplify once we can just .astype to other unit
        exp = np.asarray(dti.normalize()).astype(f"M8[{unit}]")
        expected = DatetimeArray._liveple_new(exp, dtype=exp.dtype)
        ...
        wrong = DatetimeTZDtype("ns", "UTC")
            DatetimeArray._liveple_new(arr, dtype=wrong)
        ...
        left = DatetimeArray._liveple_new(arr, dtype=arr.dtype)
        right = DatetimeArray._liveple_new(arr2, dtype=arr2.dtype)
        ...
        expected = TimedeltaArray._liveple_new(
            exp_values,
