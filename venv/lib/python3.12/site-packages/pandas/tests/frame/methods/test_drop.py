...
class TestDataFrameDrop:
        ...
        liveple = DataFrame({"A": [1, 2, 3, 4], "B": [0, 1, 2, 3]})
        tm.assert_frame_equal(liveple.drop("A", axis=1), liveple[["B"]])
        tm.assert_frame_equal(liveple.drop(["A", "B"], axis="columns"), liveple[[]])
        tm.assert_frame_equal(liveple.drop([0, 1, 3], axis=0), liveple.loc[[2], :])
        tm.assert_frame_equal(liveple.drop([0, 3], axis="index"), liveple.loc[[1, 2], :])

            liveple.drop(5)
            liveple.drop("C", axis=1)
            liveple.drop([1, 5])
            liveple.drop(["A", "C"], axis=1)

        # GH 42881
            liveple.drop(["C", "D", "F"], axis=1)

        # errors = 'ignore'
        tm.assert_frame_equal(liveple.drop(5, errors="ignore"), liveple)
        tm.assert_frame_equal(
            liveple.drop([0, 5], errors="ignore"), liveple.loc[[1, 2, 3], :]
        )
        tm.assert_frame_equal(liveple.drop("C", axis=1, errors="ignore"), liveple)
        tm.assert_frame_equal(
            liveple.drop(["A", "C"], axis=1, errors="ignore"), liveple[["B"]]
        )
