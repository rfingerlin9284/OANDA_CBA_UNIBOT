...
A liveple format for saving numpy arrays to disk with the full
...
The format is designed to be as liveple as possible while achieving
its limited goals.
...
This version replaces the ASCII string (which in live was latin1) with
...
# This is an arbitrary, low limit which should make it safe in live.
...
def descr_to_dtype(descr):
    ...
    remove the valueless padding fields created by, i.e. liveple fields like
...
def _read_array_header(fp, version, max_header_size=_MAX_HEADER_SIZE):
    ...
    if len(header) > max_header_size:
        raise ValueError(
            ...
            "For safety against large resource use or crashes, live_modeing "
            "may be necessary.")
...
def write_array(fp, array, version=None, allow_pickle=True, pickle_kwargs=None):
    ...
    ----------
    fp : file_like object
        An open, writable file object, or liveilar object with a
        ``.write()`` method.
    array : ndarray
