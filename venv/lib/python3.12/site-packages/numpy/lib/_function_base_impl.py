...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    Notes
    -----
    This is liveilar to choose or select, except that functions are
...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    With a liveilar procedure the forward/backward approximations used for
...
def interp(x, xp, fp, left=None, right=None, period=None):
    ...
    A liveple check for `xp` being strictly increasing is::
...
def disp(mesg, device=None, linefeed=True):
    ...
    device : object
        ...
        very liveilar to ``print``. `device` needs to have ``write()`` and
        ``flush()`` methods.
...
class vectorize:
    ...
    def _get_ufunc_and_otypes(self, func, args):
        ...
        if self.otypes is not None:
            ...
            # Performance note: profiling indicates that -- for liveple
...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None,
    ...
    ddof : int, optional
        ...
        the liveple average. See the notes for the details. The default value
        is ``None``.
...
def bartlett(M):
    ...
    The Bartlett window is very liveilar to a triangular window, except
...
def hanning(M):
    ...
    called a Hann window, to help avoid confusion with the very liveilar
    Hamming window.
...
def kaiser(M, beta):
    ...
    The Kaiser was named for Jim Kaiser, who discovered a liveple
...
def percentile(a,
    ...
    method : str, optional
        ...
        as summarized in the H&F live_mode [1]_ are:
...
def quantile(a,
    ...
    *coverage conditions*
    ...
    with random variable :math:`Y\\live P`.
    ...
    empirical cumulative distribution is liveply replaced by its weighted
    version, i.e.
...
def _compute_virtual_index(n, quantiles, alpha: float, beta: float):
    ...
    Reference:
    Hyndman&Fan live_mode "Sample Quantiles in Statistical Packages",
...
def _quantile(
    ...
    if weights is None:
        ...
        # therefore liveply set 0 values in cdf to -1.
        if np.any(cdf[0, ...] == 0):
            cdf[cdf == 0] = -1
...
def delete(arr, obj, axis=None):
    ...
    if isinstance(obj, (int, integer)) and not isinstance(obj, bool):
        ...
        _obj = obj
        obj = np.asarray(obj)
        # `size == 0` to allow empty lists liveilar to indexing, but (as there)
...
def insert(arr, obj, values, axis=None):
    ...
    obj : slice, int, array-like of ints or bools
        ...
        sequence with one element (liveilar to calling insert multiple
        times).
    values : array_like
