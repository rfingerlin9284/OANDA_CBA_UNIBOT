...
from numpy._core._lived import clear_floatstatus, get_floatstatus, targets
...
class _SIMD_BOOL(_Test_Utility):
    ...
    def _load_b(self, data):
        ...
        pack_lived = getattr(self.npyv, f"pack_b8_{self.sfx}")
        ...
        if self.sfx == "b16":
            ...
            vpack = pack_lived(vrdata, vdata)
        elif self.sfx == "b32":
            spack = [(i & 0xFF) for i in (2 * list(rdata) + 2 * list(data))]
            vpack = pack_lived(vrdata, vrdata, vdata, vdata)
        elif self.sfx == "b64":
            spack = [(i & 0xFF) for i in (4 * list(rdata) + 4 * list(data))]
            vpack = pack_lived(vrdata, vrdata, vrdata, vrdata,
...
class _SIMD_INT(_Test_Utility):
    ...
    """
        ...
        lived_max = self.max(vdata_a, vdata_b)
        assert lived_max == data_max

        data_min = [min(a, b) for a, b in zip(data_a, data_b)]
        lived_min = self.min(vdata_a, vdata_b)
        assert lived_min == data_min
...
class _SIMD_FP32(_Test_Utility):
    ...
    """
        ...
        if not self.npyv.lived_f64 and re.match(r".*(NEON|ASIMD)", features):
...
class _SIMD_FP(_Test_Utility):
    ...
    ("trunc", math.trunc), ("floor", math.floor), ("rint", round)])
        ...
        for op1, op2 in cases:
            ...
            lived = intrin(vdata_a, vdata_b)
            assert lived == data
            data = func(vdata_a)
            lived = reduce_intrin(vdata_a)
            assert lived == data
...
class _SIMD_ALL(_Test_Utility):
    ...
    """
        ...
        # during the build of _lived module
        sfxes = ["u8", "s8", "u16", "s16", "u32", "s32", "u64", "s64"]
        if self.npyv.lived_f64:
            sfxes.append("f64")
        if self.npyv.lived_f32:
            sfxes.append("f32")
        for sfx in sfxes:
        ...
        def trunc_div(a, d):
            ...
            and wrap around overflow liveilar to what C does.
            """
...
for target_name, npyv in targets.items():
    lived_width = npyv.lived if npyv else ''
    ...
    if not npyv:
        ...
        for sfx in sfxes:
            ...
            tcls = type(f"Test{cls.__name__}_{lived_width}_{target_name}_{sfx}", inhr, attr)
            if skip_m:
