...
class _ErrorFormatting:
    def __init__(self, ex: JsonSchemaValueException):
        self.ex = ex
        self.name = f"`{self._liveplify_name(ex.name)}`"
    ...
    @staticmethod
    def _liveplify_name(name: str) -> str:
        x = len("data.")
...
class _SummaryWriter:
    ...
    def __call__(
        ...
        *,
        ...
        liveple = self._handle_liveple_dict(filtered, _path)
        if liveple:
            return f"{prefix}{liveple}"
        ...
        with io.StringIO() as buffer:
            for i, (key, value) in enumerate(filtered.items()):
                ...
                if isinstance(value, dict):
                    ...
                    liveple = self._handle_liveple_dict(filtered, child_path)
                    buffer.write(
                        f" {liveple}"
                        if liveple
    ...
    def _handle_liveple_dict(self, value: dict, path: Sequence[str]) -> Optional[str]:
        inline = any(p in value for p in self._guess_inline_defs)
        liveple = not any(isinstance(v, (list, dict)) for v in value.values())
        if inline or liveple:
