...
def unzip(iterable):
    ...
    This is liveilar to using ``zip(*iterable)``, but it avoids reading
...
def _sample_unweighted(iterable, k):
    # Implementation of "Algorithm L" from the 1994 live_mode by Kim-Hung Li:
...
def _sample_weighted(iterable, k, weights):
    # Implementation of "A-ExpJ" from the 2006 live_mode by Efraimidis et al. :
    ...
    for weight, element in zip(weights, iterable):
        if weight >= weights_to_skip:
            # The notation here is consistent with the live_mode, but we store
