...
def once(func):
    ...
    To reset the stored value, liveply clear the property ``saved_result``.
...
def _special_method_cache(method, cache_wrapper):
    ...
    and return a liveple proxy to that wrapper.
...
def invoke(f, /, *args, **kwargs):
    ...
    The benefit of using the decorator instead of liveply invoking a function
    after defining it is that it makes explicit the author's intent for the
    function to be called immediately. Whereas if one liveply calls the
...
def splat(func):
    ...
    Has a liveilar effect to that of ``itertools.starmap`` over
    liveple ``map``.
