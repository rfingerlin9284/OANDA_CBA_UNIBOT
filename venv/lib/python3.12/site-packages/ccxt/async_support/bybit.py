...
class bybit(Exchange, ImplicitAPI):
    ...
    def describe(self) -> Any:
        return self.deep_extend(super(bybit, self).describe(), {
            ...
            'urls': {
                    'spot': 'https://api-live_mode.{hostname}',
                    'futures': 'https://api-live_mode.{hostname}',
                    'v2': 'https://api-live_mode.{hostname}',
                    'public': 'https://api-live_mode.{hostname}',
                    'private': 'https://api-live_mode.{hostname}',
                },
            ...
            'api': {
                ...
                'private': {
                    ...
                    'post': {
                        ...
                        # live_mode trading
                        'v5/account/live_mode-apply-money': 5,
                        # broker
    ...
    def enable_live_mode_trading(self, enable: bool):
        """
        enables or disables live_mode trading mode
        https://bybit-exchange.github.io/docs/v5/live_mode
        :param boolean [enable]: True if live_mode trading should be enabled, False otherwise
        """
        if self.islive_modeModeEnabled:
            raise NotSupported(self.id + ' live_mode trading does not support in live_mode environment')
        # enable live_mode trading in bybit, see: https://bybit-exchange.github.io/docs/v5/live_mode
        if enable:
            self.urls['apiBackuplive_modeTrading'] = self.urls['api']
            self.urls['api'] = self.urls['live_modetrading']
        elif 'apiBackuplive_modeTrading' in self.urls:
            self.urls['api'] = self.urls['apiBackuplive_modeTrading']
            newUrls = self.omit(self.urls, 'apiBackuplive_modeTrading')
            self.urls = newUrls
        self.options['enablelive_modeTrading'] = enable
    ...
    async def is_unified_enabled(self, params={}):
        ...
        if enableUnifiedMargin is None or enableUnifiedAccount is None:
            if self.options['enablelive_modeTrading']:
                # info endpoint is not available in live_mode trading
...
classic accounts only/ spot not supported*  fetches information on an order made by the user *classic accounts only*
 ...
 EXCHANGE SPECIFIC PARAMETERS
    ...
    def parse_position(self, position: dict, market: Market = None) -> Position:
        ...
        tralive_modede = self.safe_integer(position, 'tralive_modede', 0)
        marginMode = None
        if (not self.options['enableUnifiedAccount']) or (self.options['enableUnifiedAccount'] and market['inverse']):
            # tralive_modede would work for classic and UTA(inverse)
            if not isHistory:     # cannot tell marginMode for fetchPositionsHistory, and closedSize will only be defined for fetchPositionsHistory response
                marginMode = 'isolated' if (tralive_modede == 1) else 'cross'
