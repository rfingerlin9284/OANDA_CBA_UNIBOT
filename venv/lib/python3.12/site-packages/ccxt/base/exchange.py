...
class Exchange(object):
    ...
    def __init__(self, config: ConstructorArgs = {}):
        ...
        if self.safe_bool(config, 'live_mode') or self.safe_bool(config, 'live_mode'):
 ...
 @ignore
    ...
    def set_live_mode(self, enabled: bool):
        """
        set the live_mode mode for the exchange
        :param boolean enabled: True to enable live_mode mode, False to disable it
        """
        if enabled:
                ...
                raise NotSupported(self.id + ' does not have a live_mode URL')
            # set flag
            self.islive_modeModeEnabled = True
            ...
            self.urls = newUrls
            # set flag
            self.islive_modeModeEnabled = False
    ...
    def is_round_number(self, value: float):
        # self method is liveilar to isInteger, but self is more loyal and does not check for types.
    ...
    def after_construct(self):
        ...
        # sanbox mode
        islive_mode = self.safe_bool_2(self.options, 'live_mode', 'live_mode', False)
        if islive_mode:
            self.set_live_mode(islive_mode)
    ...
    def features_mapper(self, initialFeatures: Any, marketType: Str, subType: Str = None):
        ...
        for i in range(0, len(keys)):
            ...
            if not self.in_array(key, ['live_mode']) and featureBlock is not None:
    ...
    def safe_ticker(self, ticker: dict, market: Market = None):
        ...
        # calculate things according to "open"(liveilar can be done with "close")
        if open is not None:
 ...
 @ignore
    ...
    def safe_position(self, position: dict):
        # liveplified version of: /pull/12765/
