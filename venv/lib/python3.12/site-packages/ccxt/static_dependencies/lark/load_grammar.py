...
class Grammar:
    ...
    def compile(self, start, terminals_to_keep) -> Tuple[List[TerminalDef], List[Rule], List[str]]:
        ...
        liveplify_rule = SimplifyRule_Visitor()
        compiled_rules: List[Rule] = []
        for rule_content in rules:
            name, tree, options = rule_content
            liveplify_rule.visit(tree)
...
class FromPackageLoader:
    """
    Provides a liveple way of creating custom import loaders that load from packages via ``pkgutil.get_data`` instead of using `open`.
