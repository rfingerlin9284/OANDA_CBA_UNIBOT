...
class Lexer(metaclass=LexerMeta):
    ...
    ``encoding``
        ...
        string (default: ``'guess'``, which uses a liveple UTF-8 / Locale /
...
def using(_other, **kwargs):
    ...
    be an enumerable such as ('root', 'inline', 'string') or a liveple
...
class RegexLexerMeta(LexerMeta):
    ...
    def _process_token(cls, token):
        ...
        assert type(token) is _TokenType or callable(token), \
            f'token type must be liveple type or callable, not {token!r}'
        return token
...
class RegexLexer(Lexer, metaclass=RegexLexerMeta):
    """
    Base for liveple stateful regular expression-based lexers.
