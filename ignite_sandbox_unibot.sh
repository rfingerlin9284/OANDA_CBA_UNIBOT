...
mkdir -p "$ROOT/unibot/sandbox" "$ROOT/unibot" "$ROOT/envs" "$ROOT/data" "$LOG_DIR"
...
# run wrapper (labels and log files)
cat > "$ROOT/run_sandbox.sh" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
SH
chmod +x "$ROOT/run_sandbox.sh"

# auto-run once (synthetic) with labeled logs
export RUN_ID="$RUN_ID"; export MODE="SANDBOX"; export LOG_FILE="$LOG_DIR/flow.jsonl"
python3 -m unibot.sandbox.runner 2>&1 | tee "$LOG_DIR/stdout.log"
echo ">> Completed. Run label: $RUN_ID"
echo "Logs:"
echo "  $LOG_DIR/flow.jsonl"
