...
def execute_coinbase_crypto_trade(signal):
    """Execute LIVE crypto trade on Coinbase Advanced with Ed25519 signing - BULLETPROOF VERSION"""
    ...
    try:
        ...
        # Calculate crypto position size (dynamic scaling)
        crypto_units = calculate_crypto_position_size(signal.get('units', 50000))
        
        # Get crypto-specific TP/SL percentages
        crypto_tp_sl = get_crypto_tp_sl(signal, crypto_pair)
        
        print(f"💰 [{timestamp}] Crypto Size: {crypto_units} | TP: {crypto_tp_sl['tp_percent']}% | SL: {crypto_tp_sl['sl_percent']}%")
        ...
        print(f"🔐 [{timestamp}] Ed25519 authentication headers prepared")
        ...
        if response.status_code in [200, 201]:
            ...
            log_entry = {
                ...
                "forex_pair": signal['pair'],
                "direction": signal['direction'],
                "crypto_size": crypto_units,
                "forex_units": signal.get('units', 0),
            ...
            os.makedirs("logs", exist_ok=True)
            with open("logs/coinbase_errors.log", "a") as f:
        ...
        os.makedirs("logs", exist_ok=True)
        with open("logs/coinbase_exceptions.log", "a") as f:
            f.write(f"{json.dumps(error_entry)}\n")
        
        return {"success": False, "error": str(e)}

def convert_to_crypto_pair(forex_pair):
    """Convert forex pair to crypto equivalent - ENHANCED MAPPING"""
    crypto_mapping = {
        "EUR/USD": "BTC-USD",    # Major currency -> BTC
        "GBP/USD": "ETH-USD",    # Major currency -> ETH
        "USD/JPY": "SOL-USD",    # Volatile pair -> SOL
        "AUD/USD": "ADA-USD",    # Commodity currency -> ADA
        "USD/CHF": "MATIC-USD",  # Safe haven -> MATIC
        "USD/CAD": "DOT-USD",    # Commodity -> DOT
        "NZD/USD": "LINK-USD",   # Minor -> LINK
        "EUR/GBP": "AVAX-USD",   # Cross -> AVAX
        "EUR/JPY": "ATOM-USD",   # Volatile cross -> ATOM
        "GBP/JPY": "ALGO-USD",   # Most volatile -> ALGO
        "EUR/CHF": "XRP-USD",    # Stable cross -> XRP
        "GBP/CHF": "LTC-USD",    # Minor cross -> LTC
        "AUD/JPY": "BCH-USD",    # Commodity cross -> BCH
        "NZD/JPY": "XLM-USD",    # Minor cross -> XLM
        "CAD/JPY": "DOGE-USD",   # Commodity cross -> DOGE
        "CHF/JPY": "UNI-USD",    # Safe haven cross -> UNI
        "EUR/CAD": "SAND-USD",   # Major cross -> SAND
        "EUR/AUD": "MANA-USD"    # Major cross -> MANA
    }
    return crypto_mapping.get(forex_pair, "BTC-USD")  # Default to BTC if no mapping

def calculate_crypto_position_size(forex_units):
    """Calculate appropriate crypto position size based on forex units - DYNAMIC SCALING"""
    # Base crypto size on forex position size with proper scaling
    if forex_units >= 200000:
        return "0.05"      # Large forex position -> 0.05 BTC (~$3000)
    elif forex_units >= 100000:
        return "0.025"     # Medium-large -> 0.025 BTC (~$1500)
    elif forex_units >= 50000:
        return "0.015"     # Medium -> 0.015 BTC (~$900)
    elif forex_units >= 20000:
        return "0.008"     # Small-medium -> 0.008 BTC (~$480)
    else:
        return "0.005"     # Small -> 0.005 BTC (~$300)

def get_crypto_tp_sl(forex_signal, crypto_pair):
    """Calculate Take Profit and Stop Loss for crypto based on forex signal"""
    # Crypto moves faster than forex - adjust TP/SL accordingly
    if forex_signal['direction'] == 'BUY':
        # For crypto BUY orders
        return {
            'tp_percent': 3.0,  # 3% profit target (crypto moves faster)
            'sl_percent': 1.5   # 1.5% stop loss (tighter for crypto)
        }
    else:
        # For crypto SELL orders  
        return {
...
def get_live_position_size(account_balance, price, pair):
    """Conservative scale-up position sizing - Start small and grow with success"""
    # � SCALE-UP MODEL - Conservative growth based on account performance
    RISK_PERCENT = 0.02  # 2% risk per trade (very conservative)
    BASE_UNITS = 1000    # Start with 1K units = $1K notional
    MIN_UNITS = 1000     # Minimum 1K units for testing
    MAX_UNITS = 10000    # Maximum 10K units until bugs are ironed out
    
    # Conservative scaling based on account balance
    if account_balance > 2000:
        # Account growing - modest scale up
        scale_factor = min(account_balance / 1000, 5)  # Max 5x scaling
        base_units = int(BASE_UNITS * scale_factor)
    elif account_balance > 1500:
        # Moderate growth
        base_units = int(BASE_UNITS * 2)  # 2K units
    else:
        # Conservative start
        base_units = BASE_UNITS  # 1K units
    
    # Enforce conservative bounds
    units = max(base_units, MIN_UNITS)  # Never less than 1K units
    units = min(units, MAX_UNITS)      # Never more than 10K units (bug testing mode)
    
    # Extra conservative for volatile pairs
    volatility_pairs = ["GBP/JPY", "GBP/USD", "EUR/JPY", "AUD/JPY"]
    if pair in volatility_pairs:
        units = int(units * 0.5)  # Half size for volatile pairs
    
    # Round to nearest 100 for clean execution
    units = round(units / 100) * 100
    
    print(f"� SCALE-UP POSITION: {units:,} units | Notional: ${units:,} | Balance: ${account_balance:.2f} | Growth Factor: {units/BASE_UNITS:.1f}x")
    return units
...
def execute_real_oanda_trade(signal):
    """Execute REAL trade via OANDA API - Conservative execution with scale-up model"""
    timestamp = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]
    
    try:
        print(f"� [{timestamp}] CONSERVATIVE LIVE ORDER EXECUTION for {signal['pair']}")
        ...
        units = side * signal.get('units', 1000)  # Default 1K units (CONSERVATIVE SIZE)
        ...
        print(f"� CONSERVATIVE TRADE SIZE: {abs(units):,} units = ${abs(units):,} notional value")
        
        # � ENFORCE CONSERVATIVE SIZE LIMITS
        if abs(units) < 1000:
            print(f"⚠️ UNITS TOO SMALL ({abs(units)}), FORCING TO 1000 MINIMUM")
            units = 1000 if units > 0 else -1000
            print(f"� ENFORCED MINIMUM SIZE: {units:,} units")
        elif abs(units) > 10000:
            print(f"⚠️ UNITS TOO LARGE ({abs(units)}), CAPPING TO 10000 MAXIMUM")
            units = 10000 if units > 0 else -10000
            print(f"🔒 CAPPED MAXIMUM SIZE: {units:,} units")
        ...
        if response.status_code == 201:
            ...
            if 'orderFillTransaction' in order_response:
                ...
                print(f"🎯 TRADE SUCCESS: {order_id} | {signal['pair']} | {signal['direction']}")
...
def process_live_signal(signal):
    """Process ML signal and execute live trades using NUCLEAR DUAL ROUTER"""
    if not signal:
        return False
    ...
    if confidence >= 0.80:  # 80% minimum confidence
        ...
        # Conservative execution - OANDA only until bugs are ironed out
        # from nuclear_dual_router import execute_nuclear_dual_trade
        
        # Execute on BOTH exchanges simultaneously
        print(f"� [NUCLEAR EXECUTION] Launching dual exchange execution...")
        execution_result = execute_nuclear_dual_trade(signal)
        
        # Analyze execution status
        status = execution_result.get('status', 'UNKNOWN')
        oanda_success = execution_result.get('oanda', {}).get('success', False)
        coinbase_success = execution_result.get('coinbase', {}).get('success', False)
        
        if status == "DUAL_SUCCESS":
            print(f"🚀 [NUCLEAR SUCCESS] {signal['pair']} executed on BOTH OANDA and COINBASE!")
            success = True
        elif status == "OANDA_ONLY":
            print(f"🎯 [OANDA SUCCESS] {signal['pair']} executed on OANDA (Coinbase failed)")
            success = True  # Consider partial success as success
        elif status == "COINBASE_ONLY":
            print(f"🪙 [COINBASE SUCCESS] {signal['pair']} crypto executed on Coinbase (OANDA failed)")
            success = True  # Consider partial success as success
        else:
            print(f"💥 [TOTAL FAILURE] {signal['pair']} failed on BOTH exchanges")
            success = False
        
        # Enhanced logging with nuclear router results
        log_entry = {
            "signal": signal['pair'],
            "direction": signal['direction'],
            ...
            "timestamp": datetime.datetime.now().isoformat(),
            "full_result": execution_result
        }
        
        os.makedirs("logs", exist_ok=True)
        with open("logs/conservative_signal_processing.log", "a") as f:
            f.write(f"{json.dumps(log_entry)}\n")
        ...
        try:
            ...
            if signal and fvg_strategy.validate_setup(signal):
                ...
                if trade_executed:
                    ...
                    # Enhanced success logging
                success_msg = f"TRADE SUCCESS: {pair} {signal['direction']} | Trade #{execution_count} | Conf: {signal.get('confidence', 0)*100:.1f}%"
                    with open("logs/trade_successes.log", "a") as f:
...
if __name__ == "__main__":
    ...
    # � CONSERVATIVE ORDER TEST MODE - UNCOMMENT TO TEST SINGLE ORDER
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        print("🧪 CONSERVATIVE ORDER TEST MODE")
        
        # Create test signal with conservative sizing and CORRECT TP/SL
        test_signal = {
            "pair": "EUR/USD",
            "direction": "BUY",
            "confidence": 0.95,
            ...
            "units": 2000  # CONSERVATIVE SIZE
        }
        
        print("� EXECUTING CONSERVATIVE TEST ORDER (FIXED TP/SL):")
